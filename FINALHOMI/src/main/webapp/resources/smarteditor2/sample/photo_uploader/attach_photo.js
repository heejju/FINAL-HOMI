		...
		try{
			if( !oNavigator.ie ){
				if(!!oNavigator.safari && oNavigator.version <= 5){
					...
					bSupportDragAndDropAPI = true; // IE 10 이상이면서 다중 파일 업로드 화면이 출력되면 false로
				}
			} else {
				bSupportDragAndDropAPI = false;
			}
    ...
    function html5Upload() {	
    	...
    	sUploadURL= '/hobbyFarm/multiplePhotoUpload.photo'; 	//다중 파일 upload URL
	...
	function onAjaxError (){
 	...
 	function callFileUploader (){
 		oFileUploader = new jindo.FileUploader(jindo.$("uploadInputBox"),{
 			sUrl  : '/hobbyFarm/photoUpload.photo',	//샘플 URL입니다.
