...
<body style="background: #fff6f0;">
	<div class=firstBody>
	<form method="post" action="updateLecture.lec" onsubmit="return beforeSubmit();" enctype="Multipart/form-data">
		...
		<div class="header">
			...
			<div class="firstInfo">
				...
				<p>
					...
					<select name="hobbyNo" id='hobbyNo'><!-- DB에서 불러오기 -->
						...
						<c:forEach var="hobby" items="${ hList }">
							...
							<c:if test="${ lb.hobbyNo ne hobby.hobbyNo }">
				...
				<p>
				<b class="green">회: </b><b class="brown"><input onkeyup="numberVal(this);" name="otTuition" id='otTuition' type="Number" class="brown inputS" placeholder="수강료를 입력해주세요." style="text-align: right;" value="${ lb.otTuition }">원</b>
				</p>
				<p>
				<b class="green">회: </b><b class="brown"><input onkeyup="numberVal(this);" name="otTime" id='otTime' type="Number" class="brown inputS" placeholder="회당 시간을 입력해주세요" style="text-align: right;" value="${ lb.otTime }">시간</b>
				</p>
				<p>
				<b class="green">총횟수: </b><b class="brown"><input onkeyup="numberVal(this);" name="otAllTime" id='otAllTime' type="Number" class="brown inputS" placeholder="총횟수를 입력해주세요" style="text-align: right;" value="${ lb.otAllTime }">회</b>
				</p>
				<script>
				...
				${ lb.learner.equals('low') }
				<b class="green">수강 대상: </b>
					<b class="brown">
						<select name="learner" id='learner'><!-- DB에서 불러오기 -->
							<option style="color: lightgray" value="">선택</option>
							<c:if test="${ lb.learner.equals('low') }">
								<option value="low" selected>초급자</option>
							</c:if>
							<c:if test="${ !lb.learner.equals('low') }">
								<option value="low">초급자</option>
							</c:if>
							<c:if test="${ lb.learner.equals('low') }">
								<option value="middle" selected>중급자</option>
							</c:if>
							<c:if test="${ !lb.learner.equals('low') }">
								<option value="middle">중급자</option>
							</c:if>
							<c:if test="${ lb.learner.equals('low') }">
								<option value="high" selected>고급자</option>
							</c:if>
							<c:if test="${ !lb.learner.equals('low') }">
				...
				<div>
					<div class="place" style="vertical-align: middle" id="place0">
						...
						<div class="selectLocation" id="selectDong" style="display: inline-block;" onmouseleave="closeGugun(this);">
							<div style="display: inline-block;"><span style='vertical-align:middle;' class='lightgray'>동 선택</span></div>
			...
			<div id=ableTime> <!-- 가능한 시간 시작부분 -->
				...
				<div><!-- 좌우 버튼 날짜선택, 시간선택부분 -->
					...
					<div style="display: inline-block;">
						<input type=text class="dateSelect" readonly="readonly" value="날짜선택" onclick="">
	...
	<script>
		...
		function gugunChange(e){
			...
			$(e).next().next().children().eq(0)[0].innerHTML = "<span style='vertical-align:middle; cursor: pointer;' onclick='viewdong(this);' class='green'>전체선택</span><input id='allC' type='checkbox' class='checkboxST' onclick='checkAllBtn(this);'>";
		...
		function beforeSubmit(){
			...
			if(!isChecked){
				alert("가능한 장소를 하나이상 선택해주세요.");
				return false;
			}
			
			var allDateC = $("#allDate").val();
			if(allDateC == ""){
				alert("가능한 시간을 하나이상 추가해주세요.");
				return false;
			}
