...
<body>
	...
	<div class="wrapper">
		<form id="JoinForm" action="join.me" method="post" enctype="Multipart/form-data">
			<br><br><br>
			<div>
				<table id="JoinTable" style="width: 100%;">
					...
					<tr>
						<th rowspan="2"><div id="buttonB2">아이디</div></th>
						<td colspan="2">
							<input type="text" id="userId" name="userId" class="JoinInput" placeholder=" 아이디는 영문과 숫자로 구성되며, 6자 이상 16자 이내로 구성합니다! 아이디는 수정할 수 없으니 조심해 주세요! 또 첫글자는 반드시 영문이어야 합니다 😊" >
							<input type="hidden" id="isIdUsable" value="emptyId">
							<!-- isIdUsable에는  emptyId(빈 아이디), falseId(잘못된 아이디), dupId(중복된 아이), trueId(사용 가능한 아이디)가 값으로 주어질 수 있다.-->
						</td>
					</tr>
					<tr>
						<td colspan="2"><pre id="userIdCheck" class="joinCheck" style="text-align: right; color: red;">아이디를 입력하세요.</pre></td>
					</tr>
					<tr>
						...
						<td colspan="2">
							...
							<input type="hidden" id="isNickNameUsable" value="emptyPwd">
						</td>
					</tr>
					<tr>
					...
					<tr>
						<th rowspan="2"><div id="buttonB2">비밀번호</div></th>
						<td colspan="2">
					...
					<tr>
						<td colspan="2"><pre id="userPwdCheck" class="joinCheck" style="text-align: right; color: red;">비밀번호를 입력하세요.</pre></td>
					</tr>
					<tr>
						<th rowspan="2"><div id="buttonB2">비밀번호 확인</div></th>
						<td colspan="2">
					...
					<tr>
						<td colspan="2"><pre id="userPwd2Check" class="joinCheck" style="text-align: right; color: red;">비밀번호 확인을 입력하세요.</pre></td>
					</tr>
					<tr>
						<th rowspan="2"><div id="buttonB2" style="margin-bottom: 50%;">이메일</div></th>
						<td colspan="2">
							<input type="email" id="joinEmail" name="joinEmail" class="JoinInput" placeholder=" 실제로 사용하는 이메일을 입력해 주세요!" >
							<input type="hidden" id="isEmailUsable" value="uncheckEmail"><!-- value : falseEmail(인증번호X), trueEmail(인증번호O), uncheckEmail(인증버튼 안누름) -->
							...
							<input type="hidden" id="emailConfirmAnswer" name="emailConfirmAnswer" value="#empty">
						</td>
					</tr>
					<tr>
						<td colspan="2"><pre id="emailCheck" class="joinCheck" style="text-align: right; color: red;">이메일을 입력하세요.</pre></td>
					</tr>
					<tr>
					...
					<tr>
						<th rowspan="2"><div id="buttonB2" style="margin-bottom: 36%;">회원종류</div></th>
						<td colspan="2">
		...
		<script>
			...
			function joinSubmit() {
				...
				if($('#imgUsable').val() == "noImg") {
					...
					$('#userName').focus() ;
					isUsable = 0 ;
				} else if($('#isUserPwdUsable').val() != "truePwd") {
					setTimeout(function() {
						swal({
							title: "항목을 확인해 주세요!",
							text: "비밀번호가 잘못되었어요!",
					...
					$('#userPwd').focus() ;
					isUsable = 0 ;
				} else if($('#isUserPwd2Usable').val() != "truePwd") {
					setTimeout(function() {
						swal({
							title: "항목을 확인해 주세요!",
							text: "비밀번호가 일치하지 않아요!",
		...
		<script>
			...
			$('#userId').on('keyup', function() {
				...
				var reg = /^[A-Za-z]{1}[A-Za-z0-9]{5,15}$/ ;
				
				if($userId.trim().length == 0) {
					$('#userIdCheck').html("아이디를 입력하세요.") ;
					$('#userIdCheck').css('color', 'red') ;
					$('#isIdUsable').val("emptyId") ;
				} else {
					if(!reg.test($userId)) {
						$('#userIdCheck').html("잘못된 아이디입니다.") ;
						$('#userIdCheck').css('color', 'red') ;
						$('#isIdUsable').val("falseId") ;
					} else {
						checkDupId($userId) ;
					}
				}
			...
			function checkDupId($userId) {
				...
				$.ajax({
					...
					success: function(data) {
						if(data == 'true') {
							$('#userIdCheck').html("사용 가능한 아이디입니다.") ;
							$('#userIdCheck').css('color', 'green') ;
							$('#isIdUsable').val("trueId") ;
						} else {
							$('#userIdCheck').html("중복된 아이디입니다.") ;
							$('#userIdCheck').css('color', 'red') ;
							$('#isIdUsable').val("dupId") ;
						}
					}
				}) ;
			}
